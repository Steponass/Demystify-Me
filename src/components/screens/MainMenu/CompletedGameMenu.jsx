...
const CompletedGameMenu = ({ onLevelSelect, onStartFresh }) => {
  return (
    <div className={styles.menuContent}>
      <h1 className={styles.completed_title}>Mystify Me</h1>

      <ThresholdAdjuster />

      <div className={styles.levelSelection}>
        {/* filter levels with numerical ids to not render "tutorial" */}
        {LEVEL_METADATA.filter((level) => typeof level.id === "number").map(
          (level) => (
            <ActionButton
              key={level.title}
      ...
      <div className={styles.freshStartContainer}>
        <ActionButton variant="primary" onClick={onStartFresh}>
          Start Fresh
        </ActionButton>
      </div>
    </div>
