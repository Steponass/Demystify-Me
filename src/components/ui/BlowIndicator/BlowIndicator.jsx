...
const BlowIndicator = () => {
  ...
  // Number of samples to keep for the moving average
  const sampleSize = 10;
  ...
  // Apply moving average smoothing to the audio level
  useEffect(() => {
    // Add new audio level to the array
    audioLevelsRef.current.push(currentAudioLevel);
    
    // Keep only the most recent samples
    if (audioLevelsRef.current.length > sampleSize) {
      audioLevelsRef.current.shift();
    }
    
    // Calculate the moving average
    const sum = audioLevelsRef.current.reduce((total, level) => total + level, 0);
    const average = sum / audioLevelsRef.current.length;
    
    console.log('BlowIndicator smoothed level:', average, 'from current:', currentAudioLevel); // Debug logging
    setSmoothedLevel(average);
  }, [currentAudioLevel, sampleSize]);

  // Manage microphone based on zoom state with proper timeout handling
  ...
  return (
    ...
    className={styles.blowIndicatorContainer}>
        <svg 
          ...
          className={styles.blowSvg}
        >
          <defs>
            <clipPath id={clipPathId}>
              {/* Only show a portion based on clip percentage */}
              <rect 
                x="0" 
                y="0"
                width={`${100 - clipPercentage}%`} 
                height="100%"
              />
            </clipPath>
          </defs>
          
          {/* SVG path with clipping that reveals more as user blows */}
          <path 
            d={blowPath}
            className={styles.blowPath}
            fill="#FAFDFF"
...
export default memo(BlowIndicator);