...
const BlowIndicator = ({ 
  ...
  // Calculate fill percentage (0-100%) based on threshold
  const normalizedLevel = audioLevel / threshold;
  const fillPercentage = Math.min(normalizedLevel * 100, 100);
  
  // SVG path for the blow indicator
  ...
  // Generate a unique ID for the gradient - fixed to avoid conditional hook error
  const gradientId = "blowGradient";
  ...
  return (
    <div className={styles.blowIndicatorContainer}>
        ...
        >
          <defs>
            <linearGradient id={gradientId} x1="0%" y1="0%" x2="100%" y2="0%">
              {/* Blue section (low level) */}
              <stop offset="0%" stopColor="#007bff" />
              
              {/* Yellow section (medium level) when applicable */}
              {fillPercentage > 33 && (
                <stop offset="33%" stopColor="#ffc107" />
              )}
              
              {/* Green section (high level) when applicable */}
              {fillPercentage > 66 && (
                <stop offset="66%" stopColor="#28a745" />
              )}
              
              {/* End of colored area */}
              <stop offset={`${fillPercentage}%`} stopColor={
                fillPercentage > 66 ? '#28a745' : 
                fillPercentage > 33 ? '#ffc107' : '#007bff'
              } />
      ...
      <span className={styles.blowIndicatorText}>
